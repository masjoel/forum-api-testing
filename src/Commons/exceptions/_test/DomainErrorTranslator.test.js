...
describe('DomainErrorTranslator', () => {
  it('should translate error correctly', () => {
    ...
    expect(DomainErrorTranslator.translate(new Error('GET_THREAD.NOT_CONTAIN_NEEDED_PROPERTY')))
      .toStrictEqual(new InvariantError('tidak dapat membaca thread karena properti yang dibutuhkan tidak ada'));
    expect(DomainErrorTranslator.translate(new Error('GET_THREAD.NOT_MEET_DATA_TYPE_SPECIFICATION')))
      .toStrictEqual(new InvariantError('tidak dapat membaca thread karena tipe data tidak sesuai'));
    expect(DomainErrorTranslator.translate(new Error('GET_COMMENT.NOT_CONTAIN_NEEDED_PROPERTY')))
      .toStrictEqual(new InvariantError('tidak dapat mengambil comment karena properti yang dibutuhkan tidak ada'));
    expect(DomainErrorTranslator.translate(new Error('GET_COMMENT.NOT_MEET_DATA_TYPE_SPECIFICATION')))
     .toStrictEqual(new InvariantError('tidak dapat mengambil comment karena tipe data tidak sesuai'));
  });
